...
def init() -> None:
    ...
    for service_id in range(2, len(SERVICES) + 2):
        db.services.insert_one({'service_id': service_id, 'service_name': SERVICES[service_id-2], 'status': StatusCode.DOWN.value})
        for team_id in range(2, TEAM_COUNT + 2):
            if IPV6_ENABLED:
                ip = 'fd10::100:' + str(team_id) + ':' + str(service_id)
            else:
                ip = '10.100.' + str(team_id) + '.' + str(service_id)
            hostname = 'team' + str(team_id) + '-' + SERVICES[service_id-2].lower()
...
def check_callback(result: dict) -> None:
    ...
    db.checks.insert_one({'service': result['host']['service_name'], 'service_id': int(result['host']['service_id']), 'team_id': int(result['host']['team_id']), 'tick': result['tick'], 'host': result['host']['ip'], 'action': result['action'], 'code': result['code'], 'comment': result['comment'], 'latency': result['latency']})
    if result['action'] == 'put' and result['code'] == int(StatusCode.OK):
        db.flags.insert_one({'service':  result['host']['service_name'], 'service_id': int(result['host']['service_id']), 'team_id': int(result['host']['team_id']), 'tick': result['tick'] + 1, 'host': result['host']['ip'], 'flag': result['flag'], 'flag_id':  result['flag_id']})
