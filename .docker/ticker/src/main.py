...
def check_callback(result: dict) -> None:
    ...
    db.checks.insert_one({'service': result['host']['service_name'], 'service_id': int(result['host']['service_id']), 'team_id': int(result['host']['team_id']), 'tick': result['tick'], 'host': result['host'], 'action': result['action'], 'code': result['code'], 'comment': result['comment'], 'latency': result['latency']})
